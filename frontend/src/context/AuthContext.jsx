...
export const AuthProvider = ({ children }) => {
  ...
  const fetchUser = useCallback(async (authToken) => {
    setLoading(true);
    try {
      const res = await axios.get('https://backend-g0d7phgm7-mehedi-hasans-projects-1f9ebc78.vercel.app/api/auth/me', {
        headers: {
  ...
  const login = async ({ identifier, accountNumber, password }) => {
    try {
const res = await axios.post('https://backend-g0d7phgm7-mehedi-hasans-projects-1f9ebc78.vercel.app/api/auth/login', {
        identifier,
        accountNumber,
        password,
      });
